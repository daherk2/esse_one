...
public class LogisticaController {
	...
	public static void PesquisarFuncionario() {
		...
		if (Logistica.comboPor2.getSelectedItem().toString().equals("")) {
			...
			dados = x.Consultar("select*from motorista where numeroRegistro = '"
					+ ModelLogistica.getCodigo() + "'");
			gatilho = 1;
			Tipo = "numero de registro";
			
		}
		else if (Logistica.comboPor2.getSelectedItem().toString().equals("Nome")) {
			dados = x.Consultar("select*from motorista where nome = '"
					+ ModelLogistica.getNome() + "'");
			gatilho = 1;
			Tipo = "nome";
			...
			dados = x.Consultar("select*from motorista where cpf = '"
					+ ModelLogistica.getCpf() + "'");
			gatilho = 1;
			Tipo = "CPF";
		}
		...
		if (gatilho == 1) {
			...
			try {
				...
				ModelLogistica.setEstado(dados.getString("estado"));
				ModelLogistica.setCidade(dados.getString("cidade"));
				...
				if (ModelLogistica.getCodigo() == Integer.parseInt(dados
	...
	public static void PesquisarFuncionarioAnterior() {
		...
		ResultSet dados = x.Consultar("select*from motorista where numeroRegistro = '"
				+ (ModelLogistica.getCodigo()) + "'");
		try {
			dados.next();
			...
			JOptionPane.showMessageDialog(null, " Não há mais registros nesta direção! ",
					"Atenção", JOptionPane.WARNING_MESSAGE);
			ModelLogistica.setCodigo(String.valueOf(ModelLogistica.getCodigo() + 1));
		}
	}
	...
	public static void AtualizarFuncionario() {
		...
		String comandosql4 = "UPDATE `logistica`.`motorista` SET `nome` = '"
				+ ModelLogistica.getNome() + "', `nascimento` = '"
				+ ModelLogistica.getNascimento() + "', `nacionalidade` = '"
				+ ModelLogistica.getNacionalidade() + "', `email` = '"
				+ ModelLogistica.getEmail() + "', `rg` = '" + ModelLogistica.getRg()
				+ "', `cpf` = '" + ModelLogistica.getCpf() + "', `ctps` = '"
				+ ModelLogistica.getCarteiraTrabalho() + "', `estadoCivil` = '"
				+ ModelLogistica.getEstadoCivil() + "', `dataAdmissao` = '"
				+ ModelLogistica.getAdmissao() + "', `tipoContrato` = '"
				+ ModelLogistica.getContrato() + "', `escolaridade` = '"
				+ ModelLogistica.getEscolaridade() + "', `nomePai` = '"
				+ ModelLogistica.getNomePai() + "', `nomeMae` = '"
				+ ModelLogistica.getNomeMae() + "', `telefone1` = '"
				+ ModelLogistica.getTelefone1() + "', `telefone2` = '"
				+ ModelLogistica.getTelefone2() + "', `celular1` = '"
				+ ModelLogistica.getCelular1() + "', `celular2` = '"
				+ ModelLogistica.getCelular2() + "', `logradouro` = '"
				+ ModelLogistica.getEndereco() + "', `bairro` = '"
				+ ModelLogistica.getBairro() + "', `numeroCasa` = '"
				+ ModelLogistica.getNumero() + "', `complemento` = '"
				+ ModelLogistica.getComplemento() + "', `cep` = '"
				+ ModelLogistica.getCep() + "', `estado` = '" + ModelLogistica.getEstado()
				+ "', `cidade` = '" + ModelLogistica.getCidade() + "', `salario` = '"
				+ ModelLogistica.getSalario() + "', `numeroCnh` = '"
				+ ModelLogistica.getNumeroCnh() + "', `dataVencimento` = '"
				+ ModelLogistica.getDataVencimento()
	...
	public static void LimparFuncionario() {
		...
		Logistica.UfPesq.setText("");
		Logistica.comboCidade.setSelectedItem("");
		Logistica.CidadePesq.setText("");
	...
	public static void SalvarVeiculo() {
		...
		String comandosql = "INSERT INTO veiculo(modelo, chassi, documento, tipo, placa, anoFabricacao, licenciamento, combustivel, categoria, marca,kilometragem, cor, especie, ipva, numeroSeguro, aquisiçao,imagem) "
				+ "VALUES('"
				...
				+ "','"
				+ ModelLogistica.getAquisicao() + "','" + ModelLogistica.getImagem() + "')";
	...
	public static void ConsultarVeiculo() {
		...
		if (Logistica.comboPor.getSelectedItem().toString().equals("")) {
			...
			dados = x.Consultar("select*from veiculos where codigo = '"
					+ ModelLogistica.getCodigo1() + "'");
			gatilho = 1;
			Tipo = "O código";
		}
		else if (Logistica.comboPor.getSelectedItem().toString().equals("Modelo")) {
			dados = x.Consultar("select*from veiculos where modelo = '"
					+ ModelLogistica.getModelo() + "'");
			gatilho = 1;
			Tipo = "O modelo";
		}
		else if (Logistica.comboPor.getSelectedItem().toString().equals("Placa")) {
			dados = x.Consultar("select*from veiculos where placa = '"
					+ ModelLogistica.getPlaca() + "'");
			gatilho = 1;
			Tipo = "A placa";
		}
		...
		if (gatilho == 1) {
			...
			try {
				...
				dados.next();
				if (ModelLogistica.getCodigo1() == Integer.parseInt(dados
						.getString("codigo"))) {
					...
					ModelLogistica.setCategoria(dados.getString("nacionalImportado"));
				...
				JOptionPane.showMessageDialog(null, erro + Tipo
						+ " digitado não existe! Tente outro.    ", "Atenção",
	...
	public static void AtualizarVeiculo() {
		...
		String comandosql4 = "UPDATE `logistica`.`veiculos` SET `modelo` = '"
				+ ModelLogistica.getModelo() + "', `chassi` = '"
				+ ModelLogistica.getNumeroChassi() + "', `documento` = '"
				+ ModelLogistica.getNumeroDocumento() + "', `tipo` = '"
				+ ModelLogistica.getTipo() + "', `placa` = '" + ModelLogistica.getPlaca()
				+ "', `anoFabricacao` = '" + ModelLogistica.getAnoFabricacao()
				+ "', `licenciamento` = '" + ModelLogistica.getLicenciamento()
				+ "', `combustivel` = '" + ModelLogistica.getCombustivel()
				+ "', `categoria` = '" + ModelLogistica.getCategoria() + "', `marca` = '"
				+ ModelLogistica.getMarca() + "', `kilometragem` = '"
				+ ModelLogistica.getKilometragem() + "', `cor` = '"
				+ ModelLogistica.getCor() + "', `especie` = '"
				+ ModelLogistica.getEspecie() + "', `ipva` = '" + ModelLogistica.getIpva()
				+ "', `numeroSeguro` = '" + ModelLogistica.getSeguro()
				+ "', `aquisiçao` = '" + ModelLogistica.getAquisicao() + "', `imagem` = '"
				+ ModelLogistica.getImagem() + "' WHERE `veiculo`.`codigo` = '"
				+ ModelLogistica.getCodigo1() + "'";
		x.grava(comandosql4);
	...
	public static void PesquisarVeiculoAnterior() {
		...
		ModelLogistica.setCodigo(String.valueOf(ModelLogistica.getCodigo() - 1));
		ResultSet dados = x.Consultar("select*from veiculos where codigo = '"
				+ ModelLogistica.getCodigo1() + "'");
		try {
			dados.next();
	...
	public static void SalvarTransportadora() {
		...
		String comandosql = "INSERT INTO transportadora (razaoSocial, endereco, bairro, numero, complemento, cep, uf, cidade, cnpj, inscricaoEstadual) "
				...
				+ "','" + ModelLogistica.getCnpj() + "','" + ModelLogistica.getIe() + "')";
		x.grava(comandosql);
	}
	...
	public static void ConsultarTransportadora() {
		...
		if (Logistica.comboPor3.getSelectedItem().toString().equals("")) {
			...
			dados = x.Consultar("select*from transportadora where codigoTransportadora = '"
					+ ModelLogistica.getCodigoTransportadora() + "'");
			gatilho = 1;
			Tipo = "O código";
			
		}
		else if (Logistica.comboPor3.getSelectedItem().toString().equals("Razão Social")) {
			dados = x.Consultar("select*from transportadora where razaoSocial = '"
					+ ModelLogistica.getRazaoSocial() + "'");
			gatilho = 1;
			Tipo = "A razão social";
			...
			dados = x.Consultar("select*from transportadora where cnpj = '"
