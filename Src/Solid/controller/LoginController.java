...
public class LoginController {
	...
	public static void verificarSenha() {
		...
		ResultSet dadosl = x
				.Consultar(" select * from login1 where usuario = '"
						+ ModelLogin.getUsuario() + "' and senha = '"
						+ ModelLogin.getSenha() + "'");
		try {
			dadosl.next();
			...
			// compara o que o usuario digitou com os dados do banco
			if (usuario.equals(dadosl.getString("usuario"))
					&& senha.equals(dadosl.getString("senha"))) {
				
				janela = true;
				// compara o horário do computador com o horario
				// estabelecido
				
				if (Agora.getHours() < Long.parseLong(manha)) {
					JOptionPane.showMessageDialog(null,
							"Bom Dia, \nseja bem-vindo sr(a) "
									+ ModelLogin.getUsuario(), "Boas Vindas",
							JOptionPane.INFORMATION_MESSAGE);
				}
				
				else if (Agora.getHours() >= Long.parseLong(manha)
						&& Agora.getHours() <= Long.parseUnsignedLong(tarde)) {
					JOptionPane.showMessageDialog(null,
							"Boa Tarde, \nseja bem-vindo sr(a) "
									+ ModelLogin.getUsuario(), "Boas Vindas",
							JOptionPane.INFORMATION_MESSAGE);
