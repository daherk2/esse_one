...
public class ChatModel extends Observable {
	...
	public ChatModel(String ip, int porta) {
		this.ip = ip;
		this.porta = porta;
		...
		public void run() {
			while (true) {
				try {
					...
					Logger.getLogger(ChatModel.class.getName()).log(Level.SEVERE, null,
							ex);
				}
				erro = false;
				while (!erro) {
					DatagramPacket pacoteRecebido = new DatagramPacket(dadosReceber,
							dadosReceber.length);
					try {
						...
						try {
							...
							Logger.getLogger(ChatModel.class.getName()).log(
